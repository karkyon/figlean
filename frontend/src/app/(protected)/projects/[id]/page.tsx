/**
 * FIGLEAN Frontend - プロジェクト詳細ページ（AutoFix統合版・UI統一版）
  ...
  const loadProject = async () => {
    try {
      ...
      try {
        ...
        // predictions/suggestionsの件数を取得して追加
        try {
          const [predictionsRes, suggestionsRes] = await Promise.all([
            apiClient.get(`/analysis/${projectId}/predictions`),
            apiClient.get(`/analysis/${projectId}/suggestions`)
          ]);
          
          analysisData.predictionsCount = predictionsRes.data.data.predictions?.length || 0;
          analysisData.suggestionsCount = suggestionsRes.data.data.suggestions?.length || 0;
        } catch (err) {
          logger.warn('predictions/suggestions件数取得失敗', { projectId });
        }
        
        setAnalysisResult(analysisData);
        logger.apiSuccess('GET', `/analysis/${projectId}`, { analysisResult: analysisData });
      ...
      <main className="max-w-7xl mx-auto px-8 py-8 mt-16">
        ...
        <div className="mb-6">
          <nav className="flex gap-6 border-b overflow-x-auto">
            ...
            >
              🔮 崩壊予測
              {analysisResult?.predictionsCount !== undefined && analysisResult.predictionsCount > 0 && (
                <span className="ml-2 px-2 py-0.5 text-xs bg-purple-100 text-purple-800 rounded-full">
                  {analysisResult.predictionsCount}
                </span>
              )}
            </button>
            <button
            ...
            >
              💡 改善提案
              {analysisResult?.suggestionsCount !== undefined && analysisResult.suggestionsCount > 0 && (
                <span className="ml-2 px-2 py-0.5 text-xs bg-green-100 text-green-800 rounded-full">
                  {analysisResult.suggestionsCount}
                </span>
              )}
            </button>
            <button
